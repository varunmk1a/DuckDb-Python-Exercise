import duckdb
# Connecting to Duck and read the file
def query_minio_duckdb(bucket_name, object_name):
    try:
        ...
        result = conn.execute("SELECT * FROM data LIMIT 0;").description
        column_names = [desc[0] for desc in result]
...
query_minio_duckdb('My_bucket', 'myparquetfile.parquet')